...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <div class="text-xs text-gray-500 uppercase">Member ber-komisi</div>
        <div class="text-2xl font-bold text-gray-900 mt-1">{{ number_format($summary['total_members']) }}</div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <div class="text-xs text-gray-500 uppercase">Total Komisi</div>
        <div class="text-2xl font-bold text-indigo-600 mt-1">Rp {{ number_format($summary['total_commission'], 0, ',', '.') }}</div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <div class="text-xs text-gray-500 uppercase">Komisi Direct</div>
        <div class="text-2xl font-bold text-emerald-600 mt-1">Rp {{ number_format($summary['total_direct'], 0, ',', '.') }}</div>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <div class="text-xs text-gray-500 uppercase">Bonus Upline</div>
        <div class="text-2xl font-bold text-purple-600 mt-1">Rp {{ number_format($summary['total_upline'], 0, ',', '.') }}</div>
    </div>
</div>
