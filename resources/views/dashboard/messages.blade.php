...
<h1 class="text-2xl font-bold dk-heading mb-2">Pesan</h1>
<p class="dk-text-muted mb-4" style="font-size:14px;">Kirim pesan ke tim admin. Bisa attach gambar / PDF / dokumen (maks 5MB).</p>
            ...
            <div class="chat-bubble {{ $mine ? 'mine' : 'theirs' }}">
                ...
                @if($msg->hasAttachment())
                    <div class="chat-attachment">
                        @if($msg->isImage())
                            <a href="{{ route('dashboard.messages.attachment', $msg) }}" target="_blank">
                                <img src="{{ route('dashboard.messages.attachment', $msg) }}" alt="{{ $msg->attachment_name }}">
                            </a>
                        @else
                            <a href="{{ route('dashboard.messages.attachment', $msg) }}" class="chat-attachment-file">
                ...
                <span class="chat-meta">
                    {{ $mine ? 'Anda' : ($msg->sender->name ?? 'Admin') }} · {{ $msg->created_at->timezone(config('app.timezone'))->format('d M Y H:i') }}
                </span>
            </div>
        @empty
            <div class="chat-empty">Belum ada pesan. Tulis pesan pertama Anda untuk tim admin.</div>
        @endforelse
    </div>

    <form method="POST" action="{{ route('dashboard.messages.store') }}" enctype="multipart/form-data" class="chat-compose">
        @csrf
        ...
        <textarea name="body" rows="2" class="w-full dk-input" placeholder="Tulis pesan untuk admin..." maxlength="5000">{{ old('body') }}</textarea>
