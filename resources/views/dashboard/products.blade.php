...
<div class="dk-grid-3 gap-6" style="display:grid;grid-template-columns:repeat(3,1fr)">
    @foreach($products as $product)
        @php
            ...
            $creatorShareAmount = $commissionBasePrice * $creatorSharePercent / 100;
        ...
        <div class="dk-table">
            ...
            <div class="p-4">
                ...
                @else
                    ...
                    @if($couponDiscount > 0)
                        <p class="text-xs dk-text-muted mt-0.5">Dihitung dari harga setelah diskon kupon: Rp {{ number_format($commissionBasePrice, 0, ',', '.') }}</p>
                    @endif
                ...
                <div class="space-y-2">
                    ...
                    @else
                        <a href="{{ route('checkout', $product->slug) }}"
                           class="flex items-center justify-center gap-2 w-full px-4 py-2 rounded-lg text-sm font-semibold text-white transition-colors"
...
{{-- Upline Section --}}
@if($user->upline && $user->upline->show_upline_info)
    @php
