...
<div class="mb-6 " style="border-bottom:1px solid #1e2b3d">
    <nav class="flex space-x-8">
        <a href="{{ route('dashboard.member-products.edit', $product) }}" class="px-1 pb-3 text-sm font-medium" style="border-bottom:2px solid transparent;color:#64748b">Produk</a>
        <a href="{{ route('dashboard.products.landing-page', $product) }}" class="px-1 pb-3 text-sm font-medium" style="border-bottom:2px solid #6366f1;color:#a5b4fc">Landing Page</a>
        <a href="{{ route('dashboard.products.video-tutorials', $product) }}" class="px-1 pb-3 text-sm font-medium" style="border-bottom:2px solid transparent;color:#64748b">Video Tutorial</a>
    </nav>
</div>
...
<div class="max-w-3xl mb-8">
    <div class="dk-card" style="padding:24px;">
        ...
        <form method="POST" action="{{ route('dashboard.products.landing-page.update', $product) }}" enctype="multipart/form-data">
            @csrf @method('PUT')
            <div class="space-y-6">
                {{-- Hero Section --}}
...
<div class="max-w-3xl mb-8">
    <div class="dk-card" style="padding:24px;">
            ...
            <div class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-3 mb-6" id="gallery-grid">
                @foreach($product->landingPageImages as $image)
                    <div class="relative group rounded-lg overflow-hidden" style="border:1px solid #2d3a4a" data-id="{{ $image->id }}">
                        ...
                        @endif
                        <form method="POST" action="{{ route('dashboard.products.landing-page.images.delete', [$product, $image]) }}" class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity" onsubmit="return confirm('Hapus gambar ini?')">
                            @csrf @method('DELETE')
        ...
        <form method="POST" action="{{ route('dashboard.products.landing-page.images.upload', $product) }}" enctype="multipart/form-data">
            @csrf
            <div class="space-y-4">
                <div>
...
<div class="max-w-3xl mb-8">
    <div class="dk-card" style="padding:24px;">
                    ...
                    <div class="rounded-lg p-4" style="border:1px solid #2d3a4a;background:#151e2d">
                        <div class="flex items-start justify-between">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <form method="POST" action="{{ route('dashboard.products.landing-page.testimonials.toggle', [$product, $testimonial]) }}" class="inline">
                                    @csrf
                                ...
                                <form method="POST" action="{{ route('dashboard.products.landing-page.testimonials.delete', [$product, $testimonial]) }}" class="inline" onsubmit="return confirm('Hapus testimonial ini?')">
                                    @csrf @method('DELETE')
                        ...
                        <details class="mt-3">
                            ...
                            <form method="POST" action="{{ route('dashboard.products.landing-page.testimonials.update', [$product, $testimonial]) }}" enctype="multipart/form-data" class="mt-3 space-y-3 dk-divider pt-3">
                                @csrf @method('PUT')
                                <div class="dk-grid-2 gap-3" style="display:grid;grid-template-columns:repeat(2,1fr)">
                                    <div>
        ...
        <div class="dk-divider pt-4">
            ...
            <form method="POST" action="{{ route('dashboard.products.landing-page.testimonials.store', $product) }}" enctype="multipart/form-data">
                @csrf
                <div class="space-y-4">
