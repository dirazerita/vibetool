...
<div class="mb-6 " style="border-bottom:1px solid #1e2b3d">
    <nav class="flex space-x-8">
        ...
        <a href="{{ route('dashboard.products.landing-page', $product) }}" class="px-1 pb-3 text-sm font-medium" style="border-bottom:2px solid transparent;color:#64748b">Landing Page</a>
        <a href="{{ route('dashboard.products.video-tutorials', $product) }}" class="px-1 pb-3 text-sm font-medium" style="border-bottom:2px solid #6366f1;color:#a5b4fc">Video Tutorial</a>
    </nav>
</div>
...
<div class="max-w-3xl mb-8">
    <div class="dk-card" style="padding:24px;">
        ...
        <form method="POST" action="{{ route('dashboard.products.video-tutorials.store', $product) }}">
            @csrf
            <div class="space-y-4">
                <div>
                ...
                @foreach($tutorials as $index => $tutorial)
                    <div class="rounded-lg p-4" style="border:1px solid #2d3a4a;background:#151e2d" x-data="{ editing: false }">
                        <div x-show="!editing">
                            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px">
                                ...
                                <div style="display:flex;gap:6px;flex-shrink:0">
                                    ...
                                    <form method="POST" action="{{ route('dashboard.products.video-tutorials.toggle', [$product, $tutorial]) }}">
                                        @csrf
                                    ...
                                    <form method="POST" action="{{ route('dashboard.products.video-tutorials.destroy', [$product, $tutorial]) }}" onsubmit="return confirm('Hapus video tutorial ini?')">
                                        @csrf @method('DELETE')
                        ...
                        <div x-show="editing" x-cloak>
                            <form method="POST" action="{{ route('dashboard.products.video-tutorials.update', [$product, $tutorial]) }}">
                                @csrf @method('PUT')
                                <div class="space-y-4">
                                    <div>
